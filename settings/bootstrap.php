...
if (!function_exists('SetMetaTags')) {
	function SetMetaTags($Page, $Controller = Null) {
		...
		if ($Page->MetaRobots) $Controller->Head->AddTag('meta', array('name' => 'robots', 'content' => $Page->MetaRobots, '_sort' => 0));
...
if (!function_exists('SectionUrl')) {
	function SectionUrl($Node, $Options = False) {
...
if (!function_exists('SectionAnchor')) {
	function SectionAnchor($Node, $Attributes = Null) {
		...
		if ($NoFollowExternal) {
			static $OurHost; 
			if (is_null($OurHost)) $OurHost = Gdn::Request()->Host();
